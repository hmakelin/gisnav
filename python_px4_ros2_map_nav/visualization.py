...
class Visualization:
    ...
    def update(self, output_data: OutputData, vo_enabled: bool) -> None:
        ...
        if not output_data.pose.image_pair.mapful():
        ...
        out = np.vstack((self._map_visualization, self._vo_visualization, output_data.pose.image_pair.img.image))
        if vo_enabled:  # TODO: hack to make visualization work - try to get rid of this conditional
            if output_data.pose.image_pair.mapful():
    ...
    def _create_visualization(output_data: OutputData, display_text: str) -> np.ndarray:
        ...
        ref_img = output_data.pose.image_pair.ref.image
