...
class _AutopilotNode(_BaseNode, ABC):
    ...
    def __init__(self, name: str) -> None:
        ...
        # Publishers
        # Use name mangling to protect these from being overwritten by extending classes
        self.__vehicle_geopose_pub = self.create_publisher(GeoPoseStamped,
                                                           messaging.ROS_TOPIC_VEHICLE_GEOPOSE,
                                                           QoSPresetProfiles.SENSOR_DATA.value)
        self.__vehicle_altitude_pub = self.create_publisher(Altitude,
                                                            messaging.ROS_TOPIC_VEHICLE_ALTITUDE,
                                                            QoSPresetProfiles.SENSOR_DATA.value)
        self.__gimbal_quaternion_pub = self.create_publisher(Quaternion,
                                                             messaging.ROS_TOPIC_GIMBAL_QUATERNION,
                                                             QoSPresetProfiles.SENSOR_DATA.value)
        self.__home_geopoint_pub = self.create_publisher(GeoPointStamped,
                                                         messaging.ROS_TOPIC_HOME_GEOPOINT,
                                                         QoSPresetProfiles.SENSOR_DATA.value)

        # Subscribers
        # terrain_altitude and egm96_height properties intended to be used by extending classes -> no name mangling
        self.terrain_altitude = None
        self.__terrain_altitude_sub = self.create_subscription(Altitude,
                                                               messaging.ROS_TOPIC_TERRAIN_ALTITUDE,
                                                               self.__terrain_altitude_callback,
                                                               QoSPresetProfiles.SENSOR_DATA.value)

        self.egm96_height = None
        self.__egm96_height_sub = self.create_subscription(Float32,
    ...
    def terrain_altitude(self) -> Optional[Altitude]:
        ...
        Needed by implementing classes to generate vehicle :class:`geographic_msgs.msg.GeoPoseStamped` message
        """
    ...
    def egm96_height(self) -> Optional[Float32]:
        ...
        Needed by implementing classes to generate vehicle :class:`geographic_msgs.msg.GeoPoseStamped` and
    ...
    def publish_gimbal_quaternion(self) -> None:
        ...
        # TODO: NED or ENU? ROS convention is ENU but current implementation is actually NED?
