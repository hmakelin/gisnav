...
class Position:
    """Represents a 3D position with geographical xy-coordinates and an
...
class ContextualMapData(_ImageHolder):
    ...
    mock_data: bool = (
        False  # Indicates that this was used for field of view guess (mock map data)
    )
    altitude_scaling: Optional[
        float
    ] = None  # altitude scaling (elevation raster meters -> camera pixels)
    ...
    def _pix_to_wgs84(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Returns tuple of affine 2D transformation matrix for converting matched
        pixel coordinates to WGS84 coordinates along with intermediate transformations

        These transformations can be used to reverse the rotation and cropping
        that :func:`._rotate_and_crop_map` did to the original map.

        :return: Tuple containing 2D affine transformations from 1. pixel coordinates
            to WGS84, 2. from original unrotated and uncropped map pixel coordinates
            to WGS84, 3. from rotated map coordinates to unrotated map coordinates,
            and 4. from cropped map coordinates to uncropped (but still rotated)
        ...
        # Invert order x<->y in translation vector since height comes first in
        # Dim tuple (inputs should be Dims)
        pix_to_uncropped[0:2][:, 2] = crop_translation[::-1]

        rotation_center = map_dim_arr / 2
        rotation = cv2.getRotationMatrix2D(
            rotation_center, np.degrees(-self.rotation), 1.0
        )
        ...
        dst_corners = np.flip(
            dst_corners, axis=1
        )  # from ENU frame to WGS 84 axis order
        unrotated_to_wgs84 = cv2.getPerspectiveTransform(
            np.float32(src_corners).squeeze(), np.float32(dst_corners).squeeze()
        )

        # Ratio of boundaries in pixels and meters -> Altitude (z) scaling factor
        vertical_scaling = abs(
            gt.meter_length
        ...
        # , unrotated_to_wgs84, uncropped_to_unrotated, pix_to_uncropped
        return pix_to_wgs84_

    def _rotate_and_crop_map(self, elevation: bool = False) -> np.ndarray:
        """Rotates map counter-clockwise and then crops a dimensions-sized part
        from the middle
        ...
        # cv2.waitKey(1)
        assert (
            map_cropped.shape[0:2] == self.crop
        ), f"Cropped shape {map_cropped.shape} did not match dims {self.crop}."
        return map_cropped
    ...
    def _crop_center(img: np.ndarray, dimensions: Dim) -> np.ndarray:
        ...
        img_cropped = img[
            math.floor(cy - dimensions.height / 2) : math.floor(
                cy + dimensions.height / 2
            ),
            math.floor(cx - dimensions.width / 2) : math.floor(
...
class Pose:
    ...
    def __post_init__(self):
        ...
        if (
            np.isnan(self.r).any()
            or np.isnan(self.t).any()
            or self.r.shape != (3, 3)
            or self.t.shape != (3, 1)
        ):
            raise DataValueError(
                f"Pose input arguments were invalid: " f"{self.r}, {self.t}."
            )
...
class FOV:
    ...
    # TODO: how to estimate if fov_wgs84 is zero (cannot be projected because
    #  camera pitch too high)?
    def _estimate_altitude_scaling(self) -> float:
        """Estimates altitude scaling factor from field of view matched against
        known map

        Altitude in t is in rotated and cropped map raster pixel coordinates. We
        can use fov_pix and fov_wgs84 to find out the right scale in meters.
        Distance in pixels is computed from lower left and lower right corners
        of the field of view (bottom of fov assumed more stable than top),
        while distance in meters is computed from the corresponding WGS84
        latitude and latitude coordinates.
...
class FixedCamera:
    ...
    # TODO: refactor this class out - it was used earlier for visual odometry
    ...
    def _estimate_fov(self) -> Optional[FOV]:
        """Estimates field of view and principal point in both pixel and WGS84
        coordinates

        :return: Field of view and principal point in pixel and WGS84 coordinates,
            or None if could not estimate
        """
        assert_type(
            self.image_pair.ref, ContextualMapData
        )  # Need pix_to_wgs84, FixedCamera should have map data match
    ...
    def _estimate_attitude(self) -> Attitude:
        ...
        return Attitude(
            gimbal_estimated_attitude.as_quat(), extrinsic=self.image_pair.ref.mock_data
        )

    @staticmethod
    def _get_fov_and_c(
        img_arr_shape: Tuple[int, int], h_mat: np.ndarray
    ...
    def _estimate_position(
        ...
        """Estimates camera position (WGS84 coordinates + altitude in meters
        above mean sea level (AMSL)) as well as terrain altitude in meters.

        :param terrain_altitude_amsl: Optional ground elevation above AMSL in meters
        :param terrain_altitude_ellipsoid: Optional ground elevation above
        ...
        # Translation in WGS84 (and altitude or z-axis translation in meters
        # above ground)
        assert_type(self.image_pair.ref, ContextualMapData)  # need pix_to_wgs84
        t_wgs84 = self.image_pair.ref.pix_to_wgs84 @ np.append(
            self.camera_position[0:2], 1
        )
        t_wgs84[2] = (
            -self.fov.scaling * self.camera_position[2]
        )  # In NED frame z-coordinate is negative above ground, make altitude >0
        ...
        altitude = Altitude(
            agl=alt,
            amsl=alt + terrain_altitude_amsl
            if terrain_altitude_amsl is not None
            else None,
            ellipsoid=alt + terrain_altitude_ellipsoid
    ...
    def __post_init__(self):
        ...
        if (
            self.terrain_altitude_amsl is not None
            and self.terrain_altitude_ellipsoid is None
            or self.terrain_altitude_amsl is not None
            and self.terrain_altitude_ellipsoid is None
        ):
            raise DataValueError(
                "Please provide terrain altitude in both AMSL and above "
                "WGS 84 ellipsoid."
            )

        object.__setattr__(
            self, "h", img.camera_data.k @ np.delete(self.pose.e, 2, 1)
        )  # Remove z-column, making the matrix square
        try:
        ...
        # Raises DataValueError if can't estimate valid FOV
        fov = self._estimate_fov()
        if fov is not None:
            object.__setattr__(
                self, "fov", fov
            )  # Need to do before calling self._estimate_position
        else:
            raise DataValueError("Could not initialize a valid FixedCamera.")

        try:
            position = self._estimate_position(
                self.terrain_altitude_amsl, self.terrain_altitude_ellipsoid
            )
        ...
        if (np.abs(self.pose.t).squeeze() >= 3 * reference).any() or (
            np.abs(self.pose.t).squeeze() >= 6 * reference
        ).any():
            raise DataValueError(
                f"pose.t {self.pose.t} & pose.t {self.pose.t} have values too "
                f"large compared to (cx, cy, fx): {reference}."
            )
        ...
        if self.home_position is not None:
            object.__setattr__(
                ...
                "home",
                self.position.altitude.ellipsoid - self.home_position.altitude,
            )
...
class DataValueError(ValueError):
    """Exception returned if a valid dataclass could not be initialized from
    provided values."""
